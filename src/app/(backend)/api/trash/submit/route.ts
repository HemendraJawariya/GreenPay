...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sessionUser || sessionUser.role !== "PETUGAS") {
      return response(403, "Only officers can submit transactions");
    }
    ...
    if (existing) return response(400, "This QR Code has already been used");
    ...
    return response(500, "An error occurred while submitting the transaction");
  }
}
